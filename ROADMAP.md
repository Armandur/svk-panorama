...
**HOSTING-/DOMÄN-UTREDNING (2026-07-24, `docs/hosting-egna-domaner.md`):** två beslut, inte ett. **(A)** per team: plattform-subdomän (`<team>.svk-panorama.se`, ETT wildcard-cert, ~gratis, funkar på TERVO2 idag) vs äkta kunddomän (org = kund, plattformen på egen domän). **(B)** var plattformen bor -> HUR kunddomän-cert löses: **TERVO2 bakom NPM** kan INTE Caddy on-demand (NPM äger :443, lastbärande för ~55 hosts) men kan **eager provisionera via NPM:s REST-API** (verify-then-provision, inget handslags-glapp; NPM är statisk config-generator, on-demand-primitiven `ssl_certificate_by_lua` finns i openresty men NPM använder den ej) - vs **egen Hetzner-box (CX33 ~9 EUR/mån)** där Caddy on-demand äger :443 rent. Hemuppkoppling (Bahnhof: ej garanterat statisk IP, ingen SLA, ToS) motiverar flytt när betalande kund pekar egen domän dit. Sekvens: subdomäner nu -> kunddomäner via NPM-API på TERVO2 -> Caddy/Hetzner vid SLA-krav. `request_origin`-seamen gör flytten transparent. Punkterna nedan = ursprungsspec (antar Caddy-vägen).
