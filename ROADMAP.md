...
- [x] **"Senast ändrad" + "ändrad av" i projektlistan KLAR (2026-07-14).** Listvyn i /editor
...
- [x] **Startscen-byte uppdaterar inte visaren live i /preview - FIXAT (2026-07-16).** Byter man
...
**HOSTING-/DOMÄN-UTREDNING (2026-07-24, `docs/hosting-egna-domaner.md`).** VALT STARTLÄGE (Rasmus): **editorn** på `pano.pettersson-vik.se` på TERVO2 (single host, trivialt). **Publicerade turer** på varje teams EGNA domän (`panorama.<org>.se`) - godtyckliga kunddomäner, exakt som legacy `panorama.svenskakyrkanharnosand.se` kör NU via NPM (manuellt, proxy-host id 35). Alltså är äkta kunddomäner KÄRNAN från dag ett; frågan är bara att automatisera dagens manuella process. Vår subdomän (`<team>.pano.pettersson-vik.se`, ETT wildcard) = valfri fallback för team utan egen domän. **HUR kunddomän-cert automatiseras (Beslut B):** **TERVO2 bakom NPM** kan INTE Caddy on-demand (NPM äger :443, lastbärande för ~55 hosts) men kan **eager provisionera via NPM:s REST-API** (verify -> skapa proxy-host + cert; inget handslags-glapp; NPM är statisk config-generator, on-demand-primitiven `ssl_certificate_by_lua` finns i openresty men NPM använder den ej) - vs senare **egen Hetzner-box (CX33 ~9 EUR/mån)** där Caddy on-demand äger :443 rent. Hemuppkoppling (Bahnhof: ej garanterat statisk IP, ingen SLA, privat-ToS) motiverar flytt vid betalande kund + SLA-krav. Sekvens: editor på pano-subdomän nu -> kunddomäner via NPM-API på TERVO2 (kärnan) -> Caddy/Hetzner vid SLA-krav. `request_origin`-seamen gör flytten transparent. Punkterna nedan = ursprungsspec (antar Caddy-vägen).
...
- [x] **/admin/users: visa + filtrera på team KLAR (2026-07-14).** Super-admins
...
- [x] **Lås-baren täcker toppinnehåll FIXAT (2026-07-15).** `.editor-lock-banner` (fixed top:0) täckte
      ...
      `padding-top: var(--lock-banner-h)`; **plan-fullscreen** (`main.plan-app` 100vh/overflow hidden)
      kan inte paddas -> `margin-top` + `height: calc(100vh - var(--lock-banner-h))` i stället. Robust mot
      radbrytning (höjd mäts). Browser-verifierat (/preview + /plan: baren överst, verktygsfältet nedskjutet).
